...
function Timezones(){
    ...
    var self=this;
    var currentTZ="Etc/UTC";
    console.log("Timezones loaded");

    this.addHours = function(date, hour){
        return new Date(date.getTime() + hour*3600000);
    }

    this.currentDate = function(){
        currDate = new Date();
        return self.addHours(currDate,7);
    }

    this.stringToDate = function(string){
        date=moment(string, "YYYY-MM-DDTHH:mm:ss").toDate();
        return date;
    }

    this.dateToString = function(date){
        string=moment(date).format('YYYY-MM-DDTHH:mm:ss');
        return string;
    }

    this.changeTimezoneInDate = function(date,tz){
        var m = moment(date);
        time=m.tz(tz).format();
        time=self.stringToDate(time);
        time=new Date(time.getTime() - 25200*1000);
        return time;
    }

    this.addTimezones = function(){
        moment.tz.add({
        "zones": {
            "America/Adak": [
